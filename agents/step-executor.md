...
1. **Read the iteration file in full.** Treat every section as authoritative for this iteration. Do NOT load `PIPELINE.md` just because it exists at the pipeline root — the manifest is metadata and is not part of iteration execution unless explicitly referenced.
