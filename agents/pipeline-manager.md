...
The action is `{ "action": "run-improver", "iteration_path": "<abs>" }`. The CLI fires this for a step that reported `has_improvement_brief: true` (in a concurrent layer, once per such step, identified by `iteration_path`) and auto-emits `improver.started` around it. Spawn `pipeline-improver` (`subagent_type: "pipeline-improver"`) SYNCHRONOUSLY, passing the `improvement_brief` you parsed for THAT iteration verbatim as the whole prompt. On a run with `PP_*` pipeline variables, prepend one line — `Source iteration file: <the action's iteration_path>` — before the brief: the executor read a rendered `.runtime/…/rendered/…` copy, so paths inside its brief may point there, and the improver must edit the SOURCE. **Do NOT pass a `model` parameter** — `pipeline-improver` pins itself to Opus + `effort: max`; a per-call `model` would downgrade it. Wait for its report. Parse its `script_creation_briefs` **LIST** (0..N) and **keep it** — the next `run-script-creator` actions index into it.
