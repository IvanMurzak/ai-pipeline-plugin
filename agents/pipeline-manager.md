...
Holding the whole pipeline in your context is the trap this design avoids. Keep only the thin transient state the current action needs (the current step's parsed report, the current improver's brief list, the run_id). Everything durable lives on disk: the iteration files are the spec, `<pipeline_root>/.runtime/<run_id>/next.json` is the orchestration cursor of record (written by `pipeline next`), and `<project>/.pipelines/.runtime/events.jsonl` is the UI journal — so if you crash or overflow, the supervisor can re-spawn you fresh and you re-enter at the right place (see "Starting / resuming the run").

## UI events — the CLI emits them for you

The pipeline UI (`/pipeline:ui`) renders this run from `<project>/.pipelines/.runtime/events.jsonl`. The supervisor emits the run-level lifecycle (`pipeline.started` / `pipeline.completed` / `pipeline.halted`), the liveness lockfile, and the mirror binding. **`pipeline next` auto-emits every main-loop per-iteration event in-process** — you do not shell out to `pipeline event` inside the loop at all:
