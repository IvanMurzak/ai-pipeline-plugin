...
The pipeline UI (`/pipeline:ui`) renders this run from `<project>/.pipeline/.runtime/events.jsonl`. The supervisor emits the run-level lifecycle (`pipeline.started` / `pipeline.completed` / `pipeline.halted`), the liveness lockfile, and the mirror binding. **`pipeline next` auto-emits every main-loop per-iteration event in-process** — you do not shell out to `pipeline event` inside the loop at all:
