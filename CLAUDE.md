...
apps/
  pipeline-cli/                     # Unified local TypeScript CLI (`pipeline <command>`), run with Bun — deterministic, LLM-free work the agents shell out to
    src/cli.ts                      #   entry; subcommands: `init`, `clone`, `plan`, `match`, `event`, `route`, `next`, `drive`, `step`, `gc`, `ci-wait`, `stats` (+ `stats backfill`), `ui`, `logs`, `submodule`, `release`, `cloud`, `mesh`
    src/commands/init.ts            #   `pipeline init` — local-first setup in one command (simplified-onboarding design, task a1): install the Claude Code plugin, clone a starter pipeline (default support-answer), start the dashboard, offer to run it, print one next action. Pure COMPOSITION — clone via lib/templates.ts, the dashboard via ui.ts's runUi (stdout captured), the plugin via a `claude plugin marketplace add`/`install` shell-out, the run via drive.ts's runDrive() in-process (reformats its step.started/step.completed progress stream into a compact display); see docs/cli.md
    ...
    src/commands/department.ts      #   `pipeline department new`/`validate`/`serve`/`status`/`stop`/`retire` (simplified-onboarding tasks a8 + a9 + a10) — `new` scaffolds `department.yml` and NOTHING else (D3), current folder by default or `./<name>/` (D4), `--from-pipeline <name>` prefilling description + one skill from an EXISTING `.claude/pipeline/<name>/PIPELINE.md` (via `lib/match.ts`'s End State/Scope.In parser) and writing `engine: pipeline` + `pipelineRoot`/`startIteration`; `validate` layers Coherence/Engine/Advisory/Local findings on top of `lib/department-manifest.ts`'s (task a7) Schema/Version ones — never re-parses the manifest itself; `serve` (a9) runs 05 §5's nine steps — validate in full, the SHARED 04 auth ladder (`commands/cloud.ts`'s `authenticateApi`, `PIPELINE_MACHINE_TOKEN` included), computed digest, POST/PATCH/no-op by slug, runner enrolment + `pipeline-runner bind` + `service install` by SHELLING OUT (never writing another package's config store, D26), install claim, then `● online` / `⏸ waiting for an admin` / `○ registered — not serving`. Idempotent and resumable; writes NOTHING inside the department folder (no cloud.json — a department is clonable). `stop` (a10) is ENTIRELY LOCAL — shells `pipeline-runner unbind`, zero network, zero auth — resolving the department id by re-deriving THIS machine's own binding (`resolveLocalDepartmentId`, matched on `cwd`) and never touching the cloud registration; not-bound-here is a no-op success. `retire` (a10) is `stop` + `DELETE /api/v1/departments/:id` (owner-only), refused without `--yes` unless interactive (`--json` always counts as non-interactive, D27), reporting `failedTaskCount`. `status` (a10, own `StatusCommandDeps`) NEVER runs the interactive auth ladder — it reuses an already-stored, silently-refreshable credential (`ensureFreshCredential`) and falls back to a local-only bound/not-bound view when none is usable (DoD: works with the network down); online it adds `N running`/`M completed today`, the D30 budget line verbatim from `GET /api/v1/dept-usage` (no client arithmetic), a stale-digest flag (this runner's claimed install digest vs. the department's current one), and per-task lines from `GET /api/v1/dept-tasks?department_id=<id>` — the pre-b4 LOCAL journal carries no `department_id` at all (confirmed by reading pipeline-runner's `department/events.ts` directly) so it is deliberately never read; the cloud's task store is what makes per-task history possible today. See docs/cli.md.
    ...
    src/lib/cloud-config.ts         #   credential store + project binding for `pipeline cloud connect` — the SECRET per-user credential file (outside any project) vs. the non-secret `.claude/pipeline/cloud.json` project binding; the mesh notifier reads the same credential store (see docs/mesh-mcp.md)
    src/commands/cloud.ts           #   `pipeline cloud connect` — browser authorization_code + PKCE (RFC 8252 loopback redirect) by default, falling back to the AS's RFC 8628 device grant when no browser/port/SSH prevents it or `--device` is passed (simplified-onboarding tasks a2/a3); a THIRD, top-of-the-ladder rung, `PIPELINE_MACHINE_TOKEN`/`--machine-token` (task a4), suppresses both entirely for the no-human `client_credentials` path (bots/CI/agents) — mutually exclusive with `--device` (exit 2); writes the credential store + project binding. The ladder itself is exported as `authenticateApi()` (credential + org, NO project binding) so `pipeline department serve` (a9) authenticates through the SAME browser/device/machine flows and the SAME 04 §9 messages (D12) without writing a `cloud.json` into a clonable department folder
    src/lib/mesh-notify.ts          #   department-mesh background notifier core (task a1): poll/diff/journal against the caller's open mesh tasks via the REST dept-tasks surface (deliberate transport deviation from the /mcp tool surface — see the file's header comment), pending-notification queue drained by hooks/mesh_notifier_relay.ts; see docs/mesh-mcp.md
    src/lib/os-notify.ts            #   best-effort cross-platform OS-level toast (osascript/notify-send/PowerShell NotifyIcon) for the mesh notifier — never required for correctness, always swallows errors
    src/commands/mesh.ts            #   `pipeline mesh notify [--interval-ms] [--once] [--json]` — the notifier daemon CLI entry, normally spawned detached by hooks/mesh_notifier_relay.ts
    src/lib/department-manifest.ts  #   `department.yml` — schema, `apiVersion`, digest, and the ADVERTISED/LOCAL split (simplified-onboarding task a7). Pure: parse text → manifest + findings; `advertisedManifest()` is an ALLOW-LIST projection of everything the cloud may see (identity, skills, `communication` capabilities, `scheduling`/`limits`/`retention`) and `runtime:` is local IN FULL — the cloud is told what a department can DO, never which engine implements it, where it runs, or what it executes. `buildRegistrationRequest()` re-checks the built body against a deny-list of key names at any depth (`assertNoLocalFields`) rather than relying on the API's 400. Digest = `sha256:` over canonicalized advertised JSON, stable across key order/quoting/comments, with `apiVersion` excluded so adding it re-arms no approval; ⚠ changing the canonicalizer or any resolved default moves EVERY department's digest and must come with a new `apiVersion`. Engine → adapterId mapping lives here and never surfaces `adapterId` in user-facing text
    src/lib/department-serve.ts     #   `department serve`'s steps that leave the process (a9): the control-plane calls (list/POST/PATCH departments, claim install) and the `pipeline-runner` shell-outs (`bind`, `service install`). Holds the local runtime-binding translation (engine → adapterId + command/args/cwd/lifecycle/nested spec) and the servable-adapter list — an engine with no module in pipeline-runner is refused BEFORE anything is registered. `commands/department.ts` owns the order, the transcript, and the exit code. PLUS (a10) `status`/`stop`/`retire`'s shared machinery: `resolveLocalDepartmentId`/`readLocalBindings`/`unbindRuntime` (shell `pipeline-runner bindings --json`/`unbind`, matched on `cwd` — a department folder's own absolute path is already a unique local key, zero network), `listDepartments`/`findDepartmentBySlug` (the cloud fallback), and the `status`-only reads `fetchDepartmentProfile`/`fetchInstalls`/`fetchDeptUsage`/`fetchDeptTasks` + `retireDepartmentRequest` (DELETE). `renderState` gained a `'stopped'` variant for a10
...
hooks/
  ...
  mesh_notifier_relay.ts            # SessionStart hook (department-mesh task a1) — ensures `pipeline mesh notify` is running detached (pid-lock guarded) + drains its pending-notification journal into SessionStart additionalContext; gated by PIPELINE_MESH_NOTIFY_ENABLED (default ON), no-ops until `pipeline cloud connect` has run once; see docs/mesh-mcp.md
docs/                               # On-demand reference docs split out of CLAUDE.md — read before editing the matching subsystem
  ...
  mesh-mcp.md                       #   Department-mesh remote MCP entry + background notifier (task a1) — connect flow, notifier architecture, transport-deviation rationale, env vars
...
- Department mesh remote MCP entry + background notifier (department-mesh design, task a1) — the `plugin.json` `mcpServers` entry, the one-time OAuth connect flow, the notifier's poll/diff/journal architecture and its documented REST-vs-MCP transport deviation, OS-notify + SessionStart-context dual delivery, env vars — see `docs/mesh-mcp.md`
