#!/usr/bin/env bun
/**
 * Pipeline plugin — SessionStart journal relay.
 *
 ...
 *  in hooks/analytics_relay.ts — keep them in sync. Depth- and
...
function appendSessionOpened(projectRoot: string, worktree: string | null): void {
  ...
  const evt = {
    // Keep in sync with apps/pipeline-cli/src/lib/event.ts (v2).
    schema: 2,
