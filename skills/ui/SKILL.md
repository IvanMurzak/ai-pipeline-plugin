...
The dashboard is read-only. It never writes to project files; it only reads `<project>/.pipeline/.runtime/events.jsonl` and the pipeline manifests.
...
3. **If the current project has no `.pipeline/` directory** yet, the
...
- A `SessionStart` hook in this plugin also launches the daemon automatically when Claude Code starts in any project that has `.pipeline/`. So in practice this skill rarely needs to *start* the daemon — most users just need the URL.
