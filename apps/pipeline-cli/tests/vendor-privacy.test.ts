...
//
// Since `b23` the filter makes the SHAPE of the value the rule, so each is now
// planted BOTH ways, and the rule gets its own section at the bottom.
...
describe('vendored privacy filter — metadata tier is the trust boundary', () => {
  test('a record stuffed with prompt-like text, absolute paths, tool arguments and error excerpts carries NONE of them at metadata tier', () => {
    const events: Record<string, unknown>[] = [
      ...
      // survive untouched. Both halves are the contract; only this one was
      // ever planted, and only the other one was ever asserted.
    ...
    const relative = (filtered[7] as Record<string, unknown>).data as Record<string, unknown>;
    expect(relative.iteration_path).toBe('steps/03-review.md');
