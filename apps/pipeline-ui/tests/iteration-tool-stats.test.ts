...
// The step-identity key was renamed `step_id` → `step_name` in schema v5.
// Every named-step assertion below runs TWICE — once per spelling — because
// EVENTS.md makes the v4 fallback load-bearing: journals already on disk carry
// `step_id`, and a v5 daemon must fold them to byte-identical numbers.
const STEP_KEYS = ["step_name", "step_id"] as const;
/** `{ step_name: n }` (v5) or `{ step_id: n }` (v4) for the key under test. */
function named(key: (typeof STEP_KEYS)[number], name: string): Record<string, unknown> {
  return { [key]: name };
}

describe.each(STEP_KEYS)(
  "iterationToolStatsForRun — named-step mode, `%s` (parallel / overlap-safe)",
  (key) => {
    test("(a) overlapping parallel iterations attribute tools/tokens to the correct step", () => {
      ...
      // would have "closed" A's window and stolen all of A's later tools. Keying
      // by the step's name, each step keeps its own.
      const events: Ev[] = [
        ev("iteration.started", { iteration_path: "/p/steps/01-a.md", ...named(key, "a"), index: 1 }),
        ev("iteration.started", { iteration_path: "/p/steps/02-b.md", ...named(key, "b"), index: 1 }),
        ...
        ev("iteration.completed", { iteration_path: "/p/steps/02-b.md", ...named(key, "b"), outcome: "completed" }),
        ev("tool.called", { success: true, agent_spawn: true }), // → a
        ev("turn.usage", { input_tokens: 100, output_tokens: 40 }), // → a
        ev("iteration.completed", { iteration_path: "/p/steps/01-a.md", ...named(key, "a"), outcome: "completed" }),
      ];
    ...
    test("(c) the iteration tree shows parallel steps as distinct rows", () => {
      ...
      const events: Ev[] = [
        ev("iteration.started", { iteration_path: "/p/steps/01-a.md", ...named(key, "a") }),
        ev("tool.called", { success: true }), // → a (only a open)
        ev("iteration.started", { iteration_path: "/p/steps/02-b.md", ...named(key, "b") }),
        ev("tool.called", { success: true }), // → b (newest open)
        ev("iteration.started", { iteration_path: "/p/steps/03-c.md", ...named(key, "c") }),
        ev("tool.called", { success: true }), // → c (newest open)
        ev("iteration.completed", { ...named(key, "c"), outcome: "completed" }),
        ev("iteration.completed", { ...named(key, "b"), outcome: "completed" }),
        ev("iteration.completed", { ...named(key, "a"), outcome: "completed" }),
      ];
    ...
    test("ambient telemetry before the first iteration.started is dropped (no open window)", () => {
      const events: Ev[] = [
        ...
        ev("iteration.started", { iteration_path: "/p/steps/01.md", ...named(key, "s1") }),
...
describe("iterationToolStatsForRun — v4→v5 step-key rename", () => {
  test("`step_name` wins when an event carries both spellings", () => {
    // Not a shape any emitter writes — it pins the resolution ORDER so a
    // future writer that double-stamps during a migration can't silently
    // split one step into two buckets.
    const events: Ev[] = [
      ev("iteration.started", { iteration_path: "/p/steps/01-a.md", step_name: "a", step_id: "legacy-a" }),
      ev("tool.called", { success: true }),
      ev("iteration.completed", { step_name: "a", step_id: "legacy-a", outcome: "completed" }),
    ];
    const stats = iterationToolStatsForRun(events);
    expect(stats.map((s) => s.step_id)).toEqual(["a"]);
    expect(stats[0].tools_called).toBe(1);
  });

  test("a run whose journal spans the rename keeps ONE bucket per step", () => {
    // The realistic upgrade shape: a parked run started under a v4 daemon and
    // resumed under a v5 one. Same step, two spellings — folding them apart
    // would double-count the tree row and halve both windows' stats.
    const events: Ev[] = [
      ...
      // …upgrade happens here; the resume re-issues the same step as v5.
      ev("iteration.started", { iteration_path: "/p/steps/01-a.md", step_name: "a" }),
      ev("turn.usage", { input_tokens: 7, output_tokens: 3 }),
      ev("iteration.completed", { iteration_path: "/p/steps/01-a.md", step_name: "a", outcome: "completed" }),
    ];
    const stats = iterationToolStatsForRun(events);
  ...
  test("an empty step name falls through to the legacy window (not a bucket named \"\")", () => {
    const events: Ev[] = [
      ev("iteration.started", { iteration_path: "/p/steps/01.md", step_name: "" }),
      ev("tool.called", { success: true }),
    ];
    const stats = iterationToolStatsForRun(events);
    expect(stats).toHaveLength(1);
    expect(stats[0].step_id).toBe("/p/steps/01.md");
    expect(stats[0].tools_called).toBe(1);
  });
});

describe("iterationToolStatsForRun — legacy mode (unnamed steps, backward-compat)", () => {
  test("(b) v3 events with no step name fold by the consecutive-iteration.started window", () => {
    // Sequential chain, no step name anywhere (a v3 journal). Each step's
